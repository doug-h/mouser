...
NetMan::NetMan(bool blocking) {
  ...
  int iResult = ioctlsocket(handle, FIONBIO, &blk);
...
void NetMan::CreateClient(const char *address) {
...
bool NetMan::Listen(char *buffer, int buffer_len) {
...
int NetMan::LatencyTest() {
  ...
  auto latency = std::chrono::duration_cast<std::chrono::milliseconds>(T2 - T1);
...
void NetMan::CheckForMessages() {
  ...
  int max_iter = 10000;
  while (max_iter-- > 0) {
    iResult = recvfrom(handle, message, sizeof(message), 0, (sockaddr *)&client,
                       &clientlen);
    if (iResult == SOCKET_ERROR) {
      int code = WSAGetLastError();
      if (code == WSAEWOULDBLOCK) {
